...
// Create a mock client that returns rejected promises with helpful errors
// This prevents the app from crashing when env vars are missing
function createMockClient() {
  const errorMsg = 'Supabase environment variables are not set. Please configure NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY in Vercel.'
  
  const mockReject = () => Promise.reject(new Error(errorMsg))
  ...
  const createMockQueryBuilder = () => {
    const builder = {
      ...
      returns: () => builder,
      then: mockReject,
      catch: (fn) => Promise.reject(new Error(errorMsg)).catch(fn),
    }
    return builder
  }
  ...
  return {
    ...
    auth: {
      getUser: mockReject,
      signInWithPassword: mockReject,
      signOut: mockReject,
      signUp: mockReject,
      admin: {
        listUsers: mockReject,
        createUser: mockReject,
        updateUserById: mockReject,
      },
    },
    channel: () => ({
...
// Returns a mock client if env vars are missing (prevents app crashes)
export const supabase = new Proxy({}, {
  get(target, prop) {
    const client = getSupabaseClient()
    
    // If client is null (env vars missing), return mock client
    if (!client) {
      if (typeof window !== 'undefined' && !isInitialized) {
        console.error('⚠️ Supabase Client Error: Missing environment variables')
        ...
        isInitialized = true // Only log once
      }
  ...
  async update(id, updates, sendWhatsAppUpdate = true) {
    ...
    if (updates.status && 
      ...
      try {
        ...
        // Try to get the note from status_history if available
        // Note: This is a best-effort attempt, the note might not be available yet
        if (updates.note) {
          statusMessage += `\n\nNote: ${updates.note}`;
        }
